...
const BudgetSection = () => {
  ...
  const budgetData = [
    { name: t("budget.teaching.materials"), value: 7500, color: "#3b82f6" },
    { name: t("budget.honorariums"), value: 10000, color: "#10b981" },
    { name: t("budget.transport"), value: 10000, color: "#6366f1" },
    { name: t("budget.administration"), value: 2500, color: "#ec4899" },
  ];
  ...
  const impactData = [
    {
      ...
      breakdown: [
        { label: t("boys"), value: 60 },
        { label: t("girls"), value: 60 }
      ]
    },
    {
  ...
  return (
    <section id="budget" className="section-padding bg-gray-50 relative">
      ...
      <div className="section-container">
                ...
                {impactData.map((item, index) => (
                  <AnimatedSection key={index} delay={index * 100} animation="fade-in">
                    <Card className="p-6 border-0 shadow-md">
                      ...
                      {/* Show breakdown for participants */}
                      {item.breakdown && (
                        <div className="mt-4 pt-4 border-t border-border/50">
                          <div className="grid grid-cols-2 gap-4">
                            {item.breakdown.map((breakdownItem, idx) => (
                              <div key={idx} className="text-center">
                                <div className="text-sm text-muted-foreground">{breakdownItem.label}</div>
                                <div className="font-semibold text-lg">{breakdownItem.value}</div>
                              </div>
                            ))}
                          </div>
                        </div>
        ...
        <AnimatedSection className="mt-16 lg:mt-20 max-w-3xl mx-auto">
